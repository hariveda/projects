package com.hari.interviewprep.designpatterns.doublecheckedlockingsingletonpattern.doublechecktest;
	...
	static void checkSingleton(Singleton s, int index) {
		...
		if (s_a != A_VALUE)
			System.out.println("[" + index + "] Singleton.a not initialized "
					+ s_a);
		if (s_b != B_VALUE)
			System.out.println("[" + index + "] Singleton.b not intialized "
					+ s_b);

		if (s_c != C_VALUE)
			System.out.println("[" + index + "] Singleton.c not intialized "
					+ s_c);
